...
export const useProfileStore = defineStore('profileStore', {
    ...
    getters: {
      ...
      isSyncing: (state) => {
        const {isSyncing: traitsSyncing} = useProfileTraitsStore()
        const {isSyncing: eventsSyncing} = useProfileEventsStore()
        return traitsSyncing && eventsSyncing
      },
    //   cleanTraits: (state) => {
    ...
    actions: {
      async loadProfileForUser(attemptsRemaining = 0) {
        this.isLoading = true
